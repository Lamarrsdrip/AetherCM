...
import { addSupportMessage, userMessages, type SupportCategory } from '@/lib/support'
import { createNotice } from '@/lib/notify'
import { sendEmail } from '@/lib/email'

const CATEGORIES:SupportCategory[]=['ACCOUNT_ACCESS','MONEY','INVESTMENTS','ACCOUNT_ACTIVITY']
...
export async function POST(req:Request){
  ...
  const category:SupportCategory|undefined=CATEGORIES.includes(b.category)?b.category:undefined
  const state=await loadState(),account=ensureAccount(state,session.email)
  const message=await addSupportMessage({userId:account.id,userName:account.name,email:account.email,from:'client',text,category})
