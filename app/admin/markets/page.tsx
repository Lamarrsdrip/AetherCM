...
export default function MarketsAdmin(){
 ...
 <div className="marketAdminListV2">{ops?.shareControls?.map((c:ShareControl)=><article key={c.symbol}>
   ...
   <div className="companyFieldsV2"><label>Share price ($)<input type="number" step=".01" value={c.manualPrice} onChange={e=>setOps({...ops,shareControls:ops.shareControls.map((x:ShareControl)=>x.symbol===c.symbol?{...x,manualPrice:Number(e.target.value)}:x)})}/></label><label>Market cap / fund size ($)<input type="number" value={c.marketCap} onChange={e=>setOps({...ops,shareControls:ops.shareControls.map((x:ShareControl)=>x.symbol===c.symbol?{...x,marketCap:Number(e.target.value)}:x)})}/></label></div>
